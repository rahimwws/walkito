...
# deciding. The scaffold ran it on the pro tier for consolidation quality, but a
# ChatGPT/Codex subscription does not expose pro-tier models, so it uses the
# config default. Restore `model: openai/gpt-5.5-pro` if you add an API key.
